...
![screenshot of data retention page](./images/edit_retention_settings.png "Edit data retention page")
...
The **Data retention** tab is located in the [Data management hub](/docs/data-apis/manage-data/manage-your-data).

## Retention periods [#adjust-retention]

The following table shows the default retention settings for our [New Relic One pricing model](/docs/accounts/accounts-billing/new-relic-one-pricing-users/pricing-billing). 
...
### Requirements for adjusting retention [#requirements]

[Pricing model](/docs/accounts/original-accounts-billing/original-product-based-pricing/overview-pricing-models)-related requirements: 
...
* [New Relic One user model](/docs/accounts/original-accounts-billing/original-users-roles/overview-user-models): requires the [**Billing user** role](/docs/accounts/accounts-billing/new-relic-one-user-management/user-management-concepts/#roles).
* The [original user model](/docs/accounts/original-accounts-billing/original-users-roles/users-roles-original-user-model): requires role of Owner, Admin, or the [**Data retention manager** add-on role](/docs/accounts/original-accounts-billing/original-users-roles/users-roles-original-user-model/#add-on). 
...
If you have multiple child-accounts in your organization, you can also copy retention values from one child-account to other child-accounts. To do this, select the **Copy settings to other accounts** link, and then select the accounts you want to apply the retention values to. Note that this feature is not appliable to the parent account because retention is automatically inherited from the parent account to child-accounts unless the child-account retention is specifically modified.
...
Raw dimensional metric data is handled a little differently when it comes from APM, Prometheus remote-write API, Telemetry SDKs, and integrations built with the Telemetry SDKs. Raw sub-minute metric data is stored for 30 days. Beyond 30 days, 1-minute resolution roll-up events are created and retained for 13 months. Like with metric timeslice data, we meter and bill on the size of the raw ingest, not the subsequent rollups.
